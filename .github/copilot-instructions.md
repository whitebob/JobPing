...
Integration should be decorator/wrapper-first and non-invasive. On the Python side, ship a server-side pip package with decorator helpers for existing service entrypoints such as FastAPI handlers, backed by a framework-neutral `asyncio` API. On the JavaScript side, ship an npm package with Promise-based wrappers/decorators around arbitrary async callables so callers can opt into `JPItem` state sync without rewriting business logic or changing result ownership. The wrapper layer must not know or encode what the wrapped function does: no fetch-specific, HTTP-specific, URL-specific, or endpoint-specific assumptions. Treat wrapped calls as opaque input and opaque output; only JobPing envelope detection/boxing/unboxing belongs in the SDK.
